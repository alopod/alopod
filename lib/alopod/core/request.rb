  ...
  class Request
    ...
    def validate_params(params)
      ...
      required_params.each do |name|
        if params.send(name).nil?
          fail ArgumentError, "Missing parameter #{name}."
        end
      end
    end
    ...
    class << self
      ...
      def params_class
        ...
        if all_params.length > 0 && @params_class.nil?
          @params_class = build_params_class(all_params, param_aliases)
        end
      ...
      def build_params_class(all_params, param_aliases)
        Struct.new(*all_params) do
          ...
          define_method :[] do |key|
            key = param_aliases[key.to_sym] if param_aliases.has_key?(key.to_sym)
            param_getter(key)
          end
